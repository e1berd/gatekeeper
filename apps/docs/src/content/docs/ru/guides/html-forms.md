...
`redirect_to`, `error_redirect_to` и `mfa_redirect_to` проверяются по `ALLOWED_REDIRECT_ORIGINS`.
Цель вне списка молча заменяется на issuer, поэтому endpoint идентификации нельзя превратить в
открытый редирект.
...
Успешный вход или регистрация устанавливают следующее на домене из `COOKIE_DOMAIN`:
...
1. **Заголовок `Origin`** — сверяется с `ALLOWED_FORM_ORIGINS`. Браузер отправляет `Origin` при
   каждом `POST` формы, поэтому origin из списка принимается без дополнительных полей. `Origin`,
...
держите отправляющий origin в `ALLOWED_FORM_ORIGINS` и полагайтесь на проверку `Origin`.
...
| `untrusted_origin`              | `Origin` присутствует, но не в `ALLOWED_FORM_ORIGINS`    |
