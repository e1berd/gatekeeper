...
| `POST /form/oauth-start`    | Начать вход через провайдера — сессия ляжет в cookie                  |
| `POST /form/signed-payload` | Обменять подписанный провайдером `payload` (напр. Telegram) на сессию |
| `POST /form/refresh`        | Ротация сессии по cookie `gk_rt`; отвечает `204`                      |
