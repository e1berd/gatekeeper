...
Если `realm` не указан, клиент выбирает `master`; его создаёт bootstrap базы данных. В
поддерживаемом безопасном browser context SDK хранит токены через асинхронный Cookie Store API. В
остальных браузерах используется `localStorage`, а вне браузера — память. Для SSR и других окружений
передайте свой `storage`. Cookie Store cookie доступны JavaScript; если токены должны быть HttpOnly,
используйте `/form/*`.
